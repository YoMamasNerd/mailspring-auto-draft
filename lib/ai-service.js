...
const TONE_INSTRUCTIONS = {
  ...
  ausfuehrlich:
    'Antworte ausführlich und detailliert — gehe gründlich auf alle Punkte ein und formuliere vollständige Sätze.',
};
...
// AnythingLLM-native `textResponse`).
...
async function generateReply({
  ...
  if (isReply) {
    ...
    if (userText && userText.trim()) {
      contextParts.push(
        ...
        'Notizen oder ein angefangener Antworttext sein. Diese Vorgaben sind verbindlicher Pflicht-Inhalt:',
        'Formuliere jeden Stichpunkt vollständig zu ausformulierten Sätzen aus und übernimm alle genannten',
        'Punkte ungekürzt. Beantworte darüber hinaus die komplette Original-Mail vollständig — gehe auf jede',
        'darin gestellte Frage und jedes Thema ein, auch wenn es in den Stichpunkten nicht erwähnt wird.',
        'Die Antwort muss inhaltlich vollständig sein und darf weder Stichpunkte weglassen noch Fragen der',
        'Original-Mail unbeantwortet lassen:',
        '"""',
        userText.trim(),
        '"""'
      );
  ...
  const requestOnce = async (requestBody, tokenCb) => {
    ...
    if (!resp.ok) {
      ...
      } else if (resp.status >= 500 && /\/v1\/openai$/.test(baseUrl)) {
        hint =
          ' Der Fehler tritt im AnythingLLM-Server auf (Details stehen nur in dessen Server-Logs); ' +
          'das Plugin versucht automatisch die native Workspace-Chat-API.';
      }
  ...
  // AnythingLLM-Sonderweg: Scheitert der OpenAI-kompatible Endpunkt
  // serverseitig (5xx, typisch: 500 mit leerem Body), bleibt die native
  // Workspace-Chat-API meist funktionsfähig — sie nutzt in AnythingLLM
  // einen anderen Code-Pfad. Erkennbar an der Basis-URL …/v1/openai;
  // das "Modell" ist dort der Workspace-Slug in der URL.
  const nativeAnythingLlmRequest = async (tokenCb) => {
    const base = activeBaseUrl();
    const root = base.replace(/\/v1\/openai$/, '');
    const slug = getConfig('model').trim();
    const url = `${root}/v1/workspace/${encodeURIComponent(slug)}/${tokenCb ? 'stream-chat' : 'chat'}`;
    // Die native API nimmt genau eine Nachricht entgegen — System-Prompt,
    // Kontext und (bei Verfeinerungen) bisheriger Vorschlag werden zu einem
    // Text zusammengeführt.
    const message = messages
      .map((m) => (m.role === 'assistant' ? `Dein bisheriger Vorschlag:\n${m.content}` : m.content))
      .join('\n\n');
    ...
    try {
      ...
      throw new Error(`Verbindung zu ${url} fehlgeschlagen: ${err.message}`);
    } finally {
      clearTimeout(timeout);
    }
    ...
    if (!resp.ok) {
      const text = (await resp.text().catch(() => '')).slice(0, 300);
      throw new Error(
        `AnythingLLM-Chat-API-Fehler ${resp.status} (${url}): ` +
    ...
    try {
      ...
      throw new Error(
        `Antwort der AnythingLLM-Chat-API war kein gültiges JSON: ${rawText.slice(0, 200)}`
      );
    }
    if (json.error && json.error !== 'null') {
      throw new Error(`AnythingLLM: ${json.error}`);
    }
    return { text: (json.textResponse || '').trim(), rawSample: rawText.slice(0, 500) };
  };

  const nativeFallbackOrRethrow = async (err, tokenCb) => {
    const isAnythingLlmBase = /\/v1\/openai$/.test(activeBaseUrl());
    if (!isAnythingLlmBase || !err.status || err.status < 500 || (signal && signal.aborted)) {
      throw err;
    }
    console.info(
      `[AI Drafts] OpenAI-kompatibler Endpunkt liefert HTTP ${err.status} — ` +
        'Fallback auf die native AnythingLLM Workspace-Chat-API.'
    );
    try {
      return await nativeAnythingLlmRequest(tokenCb);
    } catch (nativeErr) {
      if (nativeErr.message === 'Generierung abgebrochen.') throw nativeErr;
      throw new Error(`${err.message}\nAuch die native AnythingLLM-Chat-API schlug fehl: ${nativeErr.message}`);
    }
  };
  ...
  try {
    ...
    if (err.status === 404 && !(signal && signal.aborted)) {
      ...
      if (!retryable) {
        result = await nativeFallbackOrRethrow(err, onToken);
      } else {
...
module.exports = {
  ...
  // Liefert die gemerkte verfügbare Update-Version — und räumt sie auf, wenn
  // sie nicht mehr neuer als die laufende Version ist (z.B. nachdem das
  // Update manuell installiert wurde und niemand den Merker gelöscht hat).
  pendingUpdateVersion() {
    const pending = getConfig('autoUpdateAvailableVersion');
    if (!pending) return '';
    if (compareVersions(pending, PLUGIN_VERSION) <= 0) {
  ...
  // Lädt das Update-ZIP herunter und installiert es direkt ins
  // Plugin-Verzeichnis (Staging + Validierung + Backup). Aktiv wird die neue
  // Version erst nach einem Reload/Neustart von Mailspring — den bietet die
  // Einstellungsseite direkt an. Schlägt die Installation fehl, bleibt das
  // ZIP als Fallback für die manuelle Installation liegen.
  async downloadAndInstallUpdate(downloadUrl) {
    try {
      ...
      const { installFromZipBuffer, copyDir, removeDir } = require('./update-installer');

      const appData =
        AppEnv.paths && AppEnv.paths.userData ? AppEnv.paths.userData : require('os').homedir();
      const updateDir = path.join(appData, 'mailspring-auto-draft-updates');
      if (!fs.existsSync(updateDir)) fs.mkdirSync(updateDir, { recursive: true });

      const version = getConfig('autoUpdateAvailableVersion') || 'latest';
      const zipPath = path.join(updateDir, `ai-reply-drafts-${version}.zip`);
      fs.writeFileSync(zipPath, buffer);

      const pluginRoot = path.resolve(__dirname, '..');
      try {
        // Backup der laufenden Version — falls das Update Probleme macht,
        // lässt sich dieser Ordner einfach zurückkopieren.
        const backupDir = path.join(updateDir, `backup-${PLUGIN_VERSION}`);
        try {
          removeDir(backupDir);
          copyDir(pluginRoot, backupDir);
        } catch (backupErr) {
          // Ein fehlgeschlagenes Backup verhindert das Update nicht.
        }

        const staging = path.join(updateDir, `staging-${version}`);
        const info = installFromZipBuffer(buffer, pluginRoot, staging);
        setConfig('autoUpdateAvailableVersion', '');
        setConfig('autoUpdateReleaseNotes', '');
        return {
          success: true,
          ...
          message:
            `Update auf v${info.version} installiert (${info.files} Dateien). ` +
            'Aktiv nach einem Reload von Mailspring.',
        };
      } catch (installErr) {
        return {
          success: true,
          ...
          zipPath,
          message:
            `Automatische Installation fehlgeschlagen (${installErr.message}). ` +
            `ZIP liegt unter: ${zipPath} — bitte über Einstellungen → Plugins → ` +
            '"Install Plugin…" manuell installieren.',
        };
      }
    } catch (err) {
