...
function isConfigured() {
  return getConfig('baseUrl').trim().length > 0 && getConfig('model').trim().length > 0;
}
...
async function listModels() {
  const baseUrl = getConfig('baseUrl').trim().replace(/\/+$/, '');
  if (!baseUrl) {
...
async function generateReply({ subject, recipient, quotedText, userText, threadContext }) {
  if (!isConfigured()) {
  ...
  const contextParts = [
    ...
    `Empfänger der Antwort (ursprünglicher Absender): ${recipient || 'unbekannt'}`,
    '',
  ];
  if (threadContext) {
    contextParts.push(
  ...
  const body = Object.assign(
    {
      ...
      messages: [
        { role: 'system', content: getConfig('systemPrompt') || DEFAULT_SYSTEM_PROMPT },
