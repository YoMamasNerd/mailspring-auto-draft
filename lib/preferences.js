...
class AiDraftPreferences extends React.Component {
  constructor(props) {
    ...
    // pendingUpdateVersion() räumt veraltete Hinweise selbst auf (z.B. nach
    // manueller Installation der angebotenen Version).
    state.autoUpdateAvailableVersion = AIService.pendingUpdateVersion();
  ...
  _importConfig = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        newState.autoUpdateAvailableVersion = AIService.pendingUpdateVersion();
  ...
  _renderField(field) {
    ...
    if (field.type === 'textarea') {
      input = e('textarea', Object.assign({}, shared, { rows: field.rows ?? 4 }));
    ...
    return e(
      'div',
      { className: field.className ? `ai-pref-field ${field.className}` : 'ai-pref-field', key: field.key },
      e('label', {}, field.label),
      e(
        'div',
