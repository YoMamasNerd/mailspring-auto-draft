...
class AiDraftPreferences extends React.Component {
  ...
  render() {
    return e(
      ...
      FIELDS.filter((f) => !['baseUrl', 'apiKey', 'model'].includes(f.key)).map((f) =>
        this._renderField(f)
      ),
        ...
        e('button', {
          ...
          onClick: async () => {
            ...
            if (result.updateAvailable && result.downloadUrl) {
              const dlResult = await AIService.downloadAndInstallUpdate(result.downloadUrl);
              if (dlResult.success) {
                alert(dlResult.message);
              } else {
                alert(`Download fehlgeschlagen: ${dlResult.error}`);
              }
            } else {
              this.setState({ autoUpdateAvailableVersion: '', autoUpdateReleaseNotes: '' });
              alert(result.reason === 'up-to-date' ? 'Bereits auf dem neuesten Stand.' : `Kein Update: ${result.reason}`);
            }
          }
        ...
        'button',
        {
          ...
          onClick: async () => {
            ...
            if (result.updateAvailable && result.downloadUrl) {
              ...
              alert(result.reason === 'up-to-date' ? 'Bereits auf dem neuesten Stand.' : `Kein Update: ${result.reason}${result.error ? ' — ' + result.error : ''}`);
            }
          }
        },
